...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pos_embed is not None:
            x = x + self.pos_embed     # (input embedding)              

        B, H, W = x.shape[0], x.shape[1], x.shape[2]
        outs = []
        for i, blk in enumerate(self.blocks):
            x = prompt[i].reshape(B, H, W, -1) + x   # 多的，每个x多一个相匹配的，相当于示意图中的Adaptori，加号对应
            x = blk(x)   # 再给到SAM transform layer
            if i in self.out_indices:
                outs.append(x)

        x = self.neck(x.permute(0, 3, 1, 2))   # 换位置
...
class PromptGenerator(nn.Module):
    def __init__(self, scale_factor, prompt_type, embed_dim, tuning_stage, depth, input_type,
        ...
        self.embedding_generator = nn.Linear(self.embed_dim, self.embed_dim//self.scale_factor)  # 线性变换
    ...
    def init_embeddings(self, x):
        N, C, H, W = x.permute(0, 3, 1, 2).shape  # 行列交换
        x = x.reshape(N, C, H*W).permute(0, 2, 1)  # 思维变成三维
    ...
    '''
    同一x不同变换得到哦两个feature，得到shared MLP和unshared MLP。
    *************仿造改***************************
    '''
    def get_prompt(self, handcrafted_feature, embedding_feature):
        ...
        prompts = [] 
        for i in range(self.depth):
            lightweight_mlp = getattr(self, 'lightweight_mlp_{}'.format(str(i)))  # ==>lightweightmlp有变
            # prompt = proj_prompt(prompt)
            prompt = lightweight_mlp(handcrafted_feature + embedding_feature)  #  lightweight_mlp==>unshared MLPi
