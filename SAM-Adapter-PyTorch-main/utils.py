...
'''
try to calculate miou
'''
def calc_miou(y_pred, y_true):
    ...
    with torch.no_grad():
        ...
        miou = intersection_sum / (union_sum + 1e-6)  # 避免除零错误
    return miou, np.array(0), np.array(0), np.array(0)


# def calc_dsc(y_pred, y_true):
#     batchsize = y_true.shape[0]
#     dsc_total = 0
...
#             # 计算预测结果和真实标签的像素数量
#             pixels_pred = pred.sum()
#             pixels_true = true.sum()

#             # 计算 DSC
#             dsc = 2 * intersection / (pixels_pred + pixels_true)

#             # 累加每个样本的 DSC
#             dsc_total += dsc

#     # 计算平均 DSC
#     dsc_avg = dsc_total / batchsize

#     return dsc_avg, np.array(0), np.array(0), np.array(0)
    # return dsc_total, np.array(0), np.array(0), np.array(0)
...
def _iou_loss(pred, target):
    ...
    # iou = 1 - (inter / union)
    iou = 1 - (inter / (union + 1e-6))  # 添加一个小的正数，避免除零错误


    return iou.mean()    # 其实就是求miou？？？！！能直接用吗?????
    
# 计算miou
def calc_miou(y_pred, y_true):
    with torch.no_grad():
        ious = _iou_loss(y_pred, y_true)
        # miou = ious.mean()
    return ious.item(), np.array(0), np.array(0), np.array(0)
...
def calc_ber(y_pred, y_true):
    ...
    with torch.no_grad():
        assert y_pred.shape == y_true.shape
        pos_err, neg_err, ber, dsc = 0, 0, 0, 0
        y_true = y_true.cpu().numpy()
        y_pred = y_pred.cpu().numpy()
        for i in range(batchsize):
            true = y_true[i].flatten()
    ...
    return pos_err / batchsize, neg_err / batchsize, (pos_err + neg_err) / 2 / batchsize, dsc
