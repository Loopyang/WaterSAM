...
def prepare_training():
    if config.get('resume') is not None: # 从之前的训练中恢复
        model = models.make(config['model']).cuda()
        optimizer = utils.make_optimizer(  # optimizer优化器
            model.parameters(), config['optimizer'])
        epoch_start = config.get('resume') + 1 # 其实就这个不同：训练起始周期
    ...
    lr_scheduler = CosineAnnealingLR(optimizer, max_epoch, eta_min=config.get('lr_min')) # 学习率调度器
    if local_rank == 0:
...
def main(config_, save_path, args):
    ...
    with open(os.path.join(save_path, 'config.yaml'), 'w') as f:
        yaml.dump(config, f, sort_keys=False) # 将配置信息保存到YAML文件中
    ...
    # prepare_training() 为训练过程准备必要的组件:模型、优化器、训练起始周期（epoch_start）、学习率调度器
    model, optimizer, epoch_start, lr_scheduler = prepare_training()
    model.optimizer = optimizer
    lr_scheduler = CosineAnnealingLR(model.optimizer, config['epoch_max'], eta_min=config.get('lr_min')) # 学习率调度器（lr_scheduler），使用的是 PyTorch 中的 CosineAnnealingLR 类。这个调度器通常用于在训练过程中动态地调整学习率。

    model = model.cuda()
    model = torch.nn.parallel.DistributedDataParallel( # DDP模块分布式训练
        model,
    ...
    model = model.module  # model = model.module: 在分布式训练中，DistributedDataParallel 包装的模型实际上是一个 DistributedDataParallel 类的实例，而不是你原始的模型。通过 model.module，你可以获取原始模型，这样你就可以在保存或加载模型时更方便地处理。
    ...
    for name, para in model.named_parameters():
        if "image_encoder" in name and "prompt_generator" not in name:
            # 对模型中名为 "image_encoder" 且不包含 "prompt_generator" 的层的参数进行梯度冻结。这意味着这些参数在反向传播过程中将不会被更新。通常，这样的操作在迁移学习或模型微调中用于保持预训练层的权重固定，只更新模型的特定部分。
            para.requires_grad_(False)
    if local_rank == 0: # 在分布式训练环境中，local_rank 为 0 的进程（通常是主进程）执行：（保存参数）
